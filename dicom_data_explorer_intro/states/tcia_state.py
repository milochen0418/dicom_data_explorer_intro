import reflex as rx
from dicom_data_explorer_intro.services.tcia_service import (
    fetch_collections,
    fetch_modalities,
    fetch_body_parts,
    fetch_series,
...
class TCIAState(rx.State):
    ...
    def load_initial_data(self):
        ...
        self.collections = sorted(cols, key=lambda x: x.get("Collection", ""))
        mods = fetch_modalities()
        self.modalities = sorted([m.get("Modality") for m in mods if m.get("Modality")])
        parts = fetch_body_parts()
        self.body_parts = sorted(
    ...
    def update_filters(self, key: str, value: str):
        """Update a specific filter and reload dependent options if needed."""
        if key == "collection":
