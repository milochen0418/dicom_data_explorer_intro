...
def filter_sidebar() -> rx.Component:
    return rx.el.div(
        rx.el.div(
            rx.el.h2("Search Filters", class_name="font-bold text-gray-900"),
            rx.el.button(
                "Clear",
                on_click=TCIAState.clear_search,
        ...
        rx.el.div(
            ...
            rx.el.select(
                ...
                rx.foreach(
                    TCIAState.collections,
                    lambda c: rx.el.option(c["Collection"], value=c["Collection"]),
                ),
                value=TCIAState.selected_collection,
                on_change=lambda val: TCIAState.update_filters("collection", val),
        ...
        rx.el.div(
            ...
            rx.el.select(
                ...
                rx.foreach(TCIAState.modalities, lambda m: rx.el.option(m, value=m)),
                value=TCIAState.selected_modality,
                on_change=lambda val: TCIAState.update_filters("modality", val),
                class_name="w-full rounded-lg border-gray-300 border p-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 bg-white appearance-none",
            ),
            class_name="mb-4",
        ),
        rx.el.div(
            ...
            rx.el.select(
                ...
                rx.foreach(TCIAState.body_parts, lambda b: rx.el.option(b, value=b)),
                value=TCIAState.selected_body_part,
                on_change=lambda val: TCIAState.update_filters("body_part", val),
                class_name="w-full rounded-lg border-gray-300 border p-2 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 bg-white appearance-none",
            ),
            class_name="mb-6",
        ),
        rx.el.button(
            "Search Series",
            on_click=TCIAState.search_data,
    ...
    return rx.el.tr(
        ...
        rx.el.td(
            rx.el.div(
                rx.el.button(
                    "Details",
                    on_click=TCIAState.select_series(series["SeriesInstanceUID"]),
                    class_name="text-blue-600 hover:text-blue-900 font-medium text-xs mr-3",
                ),
                rx.el.button(
                    rx.icon("shopping-cart", size=16),
                    on_click=DownloadState.add_to_cart(series, "TCIA"),
        ...
        class_name=rx.cond(
            TCIAState.selected_series_uid == series["SeriesInstanceUID"],
...
def metadata_panel() -> rx.Component:
    return rx.cond(
        TCIAState.selected_series_uid != "",
        rx.el.div(
            rx.el.div(
                rx.el.h3("Series Metadata", class_name="font-bold text-gray-900"),
                rx.el.button(
                    rx.icon("x", size=18),
                    on_click=TCIAState.select_series(""),
            ...
            rx.el.div(
                rx.foreach(
                    TCIAState.selected_series_details.entries(),
            ...
            rx.el.button(
                ...
                on_click=DownloadState.add_to_cart(
                    TCIAState.selected_series_details, "TCIA"
                ),
...
def results_view() -> rx.Component:
    return rx.el.div(
        rx.el.div(
            rx.el.h1("Search Results", class_name="text-2xl font-bold text-gray-900"),
            rx.el.span(
                f"Page {TCIAState.page} of {TCIAState.total_pages}",
        ...
        rx.cond(
            ...
            rx.el.div(
                rx.spinner(size="3"),
                rx.el.p(
                    "Fetching series from TCIA...", class_name="mt-4 text-gray-500"
                ),
                class_name="flex flex-col items-center justify-center h-64",
            ),
            rx.cond(
                TCIAState.series_results.length() > 0,
                rx.el.div(
                    rx.el.div(
                        rx.el.table(
                            rx.el.thead(
                            ...
                            rx.el.tbody(
                                rx.foreach(
                                    TCIAState.current_page_results,
                    ...
                    rx.el.div(
                        rx.el.button(
                            "Previous",
                            on_click=TCIAState.set_page(TCIAState.page - 1),
                            disabled=TCIAState.page <= 1,
                        ...
                        rx.el.button(
                            "Next",
                            on_click=TCIAState.set_page(TCIAState.page + 1),
                            disabled=TCIAState.page >= TCIAState.total_pages,
                            class_name="px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed",
                        ),
                ...
                rx.el.div(
                    ...
                    rx.el.p(
                        rx.cond(
                            ...
                            "Select filters on the left and click Search Series.",
                        ),
